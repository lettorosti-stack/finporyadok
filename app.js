...
const views = {
  dashboard: ["ФинПорядок", "Главное о деньгах семьи на одном экране."],
  transactions: ["Операции", "Доходы, расходы и переводы с понятными деталями."],
  accounts: ["Счета", "Карты, наличные, вклады и остатки."],
  budgets: ["Бюджеты", "Лимиты по категориям и проектам."],
  alimony: ["Алименты", "Начисления, поступления, задолженность и документы."],
  shopping: ["Покупки", "Список покупок и сравнение цен по магазинам."],
  reports: ["Финансовая картина", "Доли расходов, проекты и динамика по периодам."],
  import: ["Импорт", "Загрузка CSV и банковских PDF-выписок."],
  settings: ["Настройки", "Хранение, экспорт и безопасность."]
};
...
function renderTransactions() {
  ...
  if (byId("txSummaryIncome")) byId("txSummaryIncome").textContent = money(income);
  if (byId("txSummaryExpense")) byId("txSummaryExpense").textContent = money(expense);
  if (byId("txSummaryNet")) {
    byId("txSummaryNet").textContent = money(net);
    byId("txSummaryNet").className = net >= 0 ? "good" : "bad";
  }
...
function extractPdfTextOperatorsWithCMap(text, cmap) {
  ...
  const pushHex = (hex) => {
    const bytes = hex.trim().match(/[0-9a-fA-F]{2}/g)?.map((x) => parseInt(x, 16)) || [];
    const decoded = decodePdfCMapBytes(bytes, cmap) || decodePdfHex(hex);
    if (decoded.trim()) chunks.push(decoded);
  };
  text.replace(/\(((?:\\.|[^\)])*)\)\s*Tj/g, (_, body) => { pushLiteral(body); return ""; });
  text.replace(/<([0-9a-fA-F\s]+)>\s*Tj/g, (_, body) => { pushHex(body); return ""; });
  text.replace(/\[((?:.|\n)*?)\]\s*TJ/g, (_, body) => {
    let line = "";
...
function findPdfFlateStreams(raw, bytes) {
  const streams = [];
  const streamRegex = /\bstream\r?\n/g;
  let match;
  while ((match = streamRegex.exec(raw))) {
    const headerStart = Math.max(0, match.index - 1600);
    const header = raw.slice(headerStart, match.index);
    if (!/\/FlateDecode\b/.test(header)) continue;
    if (/\/Subtype\s*\/Image|\/BitsPerComponent|\/ColorSpace/.test(header)) continue;

    const start = match.index + match[0].length;
    const end = raw.indexOf("endstream", start);
    if (end < 0) continue;
    const objectMatch = header.match(/(?:^|\n)(\d+)\s+0\s+obj\b(?![\s\S]*\n\d+\s+0\s+obj\b)/);
    streams.push({
      header,
...
function findPdfObjectBody(raw, objectId) {
  const match = raw.match(new RegExp(`(?:^|\\n)${objectId}\\s+0\\s+obj([\\s\\S]*?)endobj`));
  return match ? match[1] : "";
}

function parsePdfFontCMaps(raw, streamMap) {
  const fontCMaps = new Map();
  const fontRegex = /\/(F\d+)\s+(\d+)\s+0\s+R/g;
  let match;
  while ((match = fontRegex.exec(raw))) {
    const [, fontName, objectId] = match;
    if (fontCMaps.has(fontName)) continue;
    const fontBody = findPdfObjectBody(raw, objectId);
    const toUnicode = fontBody.match(/\/ToUnicode\s+(\d+)\s+0\s+R/);
    if (!toUnicode) continue;
    const cmapText = streamMap.get(toUnicode[1]) || "";
    const cmap = parsePdfCMaps(cmapText);
    if (cmap.size) fontCMaps.set(fontName, cmap);
  }
  return fontCMaps;
}

function extractPdfTextOperatorsWithFontCMaps(text, fontCMaps, fallbackCMap) {
  const chunks = [];
  let currentMap = fallbackCMap;
  const decodeHexWithCurrentFont = (hex) => {
    const bytes = hex.trim().match(/[0-9a-fA-F]{2}/g)?.map((x) => parseInt(x, 16)) || [];
    return decodePdfCMapBytes(bytes, currentMap) || decodePdfHex(hex);
  };
  const decodeLiteralWithCurrentFont = (literal) => {
    const bytes = pdfLiteralBytes(literal);
    return decodePdfCMapBytes(bytes, currentMap) || decodePdfLiteral(literal);
  };
  const tokenRegex = /\/(F\d+)\s+[-\d.]+\s+Tf|\(((?:\\.|[^\)])*)\)\s*Tj|<([0-9a-fA-F\s]+)>\s*Tj|\[((?:.|\n)*?)\]\s*TJ/g;
  let match;
  while ((match = tokenRegex.exec(text))) {
    if (match[1]) {
      currentMap = fontCMaps.get(match[1]) || fallbackCMap;
      continue;
    }
    if (match[2] !== undefined) {
      const decoded = decodeLiteralWithCurrentFont(match[2]);
      if (decoded.trim()) chunks.push(decoded);
      continue;
    }
    if (match[3] !== undefined) {
      const decoded = decodeHexWithCurrentFont(match[3]);
      if (decoded.trim()) chunks.push(decoded);
      continue;
    }
    if (match[4] !== undefined) {
      let line = "";
      match[4].replace(/\(((?:\\.|[^\)])*)\)|<([0-9a-fA-F\s]+)>/g, (_, lit, hex) => {
        line += hex ? decodeHexWithCurrentFont(hex) : decodeLiteralWithCurrentFont(lit);
        return "";
      });
      if (line.trim()) chunks.push(line);
    }
...
async function extractPdfTextLegacy(file) {
  ...
  const streams = [];
  const flateStreams = findPdfFlateStreams(raw, bytes);
  for (const stream of flateStreams) {
    const inflated = await inflatePdfStream(stream.bytes);
    if (inflated && (/begincmap|beginbfchar|beginbfrange|\bTj\b|\bTJ\b|BT|ET/.test(inflated))) {
      streams.push({ objectId: stream.objectId, text: inflated });
    }
  }
  const streamMap = new Map(streams.filter((stream) => stream.objectId).map((stream) => [stream.objectId, stream.text]));
  const cmap = new Map();
  streams.forEach((stream) => {
    for (const [key, value] of parsePdfCMaps(stream.text)) cmap.set(key, value);
  });
  const fontCMaps = parsePdfFontCMaps(raw, streamMap);
  const parts = [
    extractPdfTextOperatorsWithFontCMaps(raw, fontCMaps, cmap),
    extractPdfTextOperatorsWithCMap(raw, cmap),
    extractPdfTextOperators(raw),
    ...streams.flatMap((stream) => [
      extractPdfTextOperatorsWithFontCMaps(stream.text, fontCMaps, cmap),
      extractPdfTextOperatorsWithCMap(stream.text, cmap),
      extractPdfTextOperators(stream.text)
    ])
  ];
  return parts.join("\n").replace(/\u0000/g, "").replace(/\u00a0/g, " ");
}
...
async function extractPdfText(file) {
  const preferOfflineParser = location.protocol === "file:" || location.protocol === "app:" || navigator.onLine === false;
  if (preferOfflineParser) {
    const offlineText = await extractPdfTextLegacy(file);
    if (offlineText.trim()) return offlineText;
  }
  try {
...
function parseSberStatementText(text, source) {
  ...
  for (let i = 0; i < lines.length - 5; i += 1) {
    ...
    const key = [lines[i], amount, authCode, normalizeOperationText(description)].join("|");
